document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateSurveyLanguage(language) {
        ...
        const individualQuestions = [
            'unstable-vege-price', // ADDED: Now includes unstable vegetable question
    ...
    function updateOptionTexts(langData) {
        ...
        updateRadioOptions('prototypeInterest', ['yes', 'no'], langData);
    }
    ...
    function updateVeggieNames(langData) {
        ...
        veggieNames.forEach(veggie => {
            // Update favorite veggie cards
            const favoriteCard = document.querySelector(`.veggie-card[data-veggie="${veggie}"]`);
            if (favoriteCard) {
                const nameElement = favoriteCard.querySelector('.veggie-name');
                if (nameElement && langData[veggie]) {
                    nameElement.textContent = langData[veggie];
                }
            }

            // Update unstable veggie cards - fixed selector to use data-veggie attribute
            const unstableCard = document.querySelector(`.veggie-card.unstable[data-veggie="${veggie}"]`);
            if (unstableCard) {
                const nameElement = unstableCard.querySelector('.veggie-name');
                if (nameElement && langData[veggie]) {
                    nameElement.textContent = langData[veggie];
                }
            }
    ...
    function setupEnhancedVeggieSelection() {
        ...
        const otherInput = document.getElementById('favoriteVegOther');
        let prevOtherValue = otherInput ? otherInput.value : '';
        ...
        function updateSelectionUI() {
            ...
            if (selectionProgressEl) {
                ...
                selectionProgressEl.style.background = (selectedCount === maxVeggieSelections)
                    ? 'linear-gradient(90deg, #ff6b6b, #ff5252)'
                    : 'linear-gradient(90deg, #4CAF50, #45a049)';
            }

            // If user selected max number of cards, remove/hide the "Other" input entirely
            if (otherContainer) {
                otherContainer.style.display = (selectedCount >= maxVeggieSelections) ? 'none' : '';
            }
        }
        ...
        veggieCards.forEach(card => {
            card.addEventListener('click', () => {
                ...
                const otherFilled = otherInput && otherInput.value.trim().length > 0 ? 1 : 0;
                ...
                if (checkbox.checked) {
                    ...
                    // If we dropped below max, ensure Other input is visible again
                    if (otherContainer && selectedCount < maxVeggieSelections) {
                        otherContainer.style.display = '';
                    }
                } else {
                    // Prevent selecting if it would exceed total limit (cards + other)
                    if (selectedCount + otherFilled >= maxVeggieSelections) {
                        showPopup(`You can only select up to ${maxVeggieSelections} vegetables total (including "Other").`);
                        return;
                    }
                    // Select
                    ...
                    // If we've reached the max by selecting cards, hide the Other input
                    if (otherContainer && selectedCount >= maxVeggieSelections) {
        ...
        function initializeFromCheckboxes() {
            ...
            hiddenCheckboxes.forEach(checkbox => {
                if (checkbox.checked) {
                    selectedCount++;
                    // scope to the favorite veggie grid to avoid selecting unstable cards
                    const card = document.querySelector(`#veggie-selection .veggie-card[data-veggie="${checkbox.value}"]`);
                    if (card) card.classList.add('selected');
                }
            });

            // If Other already has a value, enforce limits immediately
            if (otherInput && otherInput.value.trim().length > 0) {
                prevOtherValue = otherInput.value;
            }

            // Hide other input if max already reached on load
            if (otherContainer && selectedCount >= maxVeggieSelections) {
                otherContainer.style.display = 'none';
            }
        ...
        // Prevent Other input when it would push total above limit
        if (otherInput) {
            otherInput.addEventListener('input', (e) => {
                const val = otherInput.value;
                const trimmed = val.trim();
                const otherWillCount = trimmed.length > 0 ? 1 : 0;

                if (otherWillCount && selectedCount + otherWillCount > maxVeggieSelections) {
                    // Revert to previous value and notify user
                    otherInput.value = prevOtherValue || '';
                    showPopup(`You already selected ${selectedCount} vegetables. Remove one to add "Other".`);
                    return;
                }

                // Save previous value only when valid
                prevOtherValue = otherInput.value;
                updateSelectionUI();
            });
        }
    ...
    // Unstable Vegetable Selection Logic
    function setupUnstableVeggieSelection() {
        ...
        // Create mapping between cards and checkboxes using existing unstableCheckboxes
        const unstableMap = new Map();
        unstableCards.forEach(card => {
            // use data-veggie attribute (cards use data-veggie in HTML)
            const veggieName = card.getAttribute('data-veggie');
            if (!veggieName) return;

            // Try to find an existing hidden checkbox for this veggie
            let checkbox = document.querySelector(`input[name="unstableVeggie"][value="${veggieName}"]`);
            if (!checkbox) {
                // fallback: create a hidden checkbox and append to the form for submission
                checkbox = document.createElement('input');
                checkbox.type = 'checkbox';
                checkbox.name = 'unstableVeggie';
                checkbox.value = veggieName;
                checkbox.style.display = 'none';
                document.getElementById('surveyForm').appendChild(checkbox);
            }
        ...
        unstableCards.forEach(card => {
            card.addEventListener('click', () => {
                const veggieName = card.getAttribute('data-veggie');
                const checkbox = unstableMap.get(veggieName);
                if (!checkbox) return;
    ...
    if (nextButton1) {
        nextButton1.addEventListener('click', () => {
            ...
            // Check veggie selections OR "Other" text input
            const veggieCheckedCount = document.querySelectorAll('input[name="favoriteVeggie"]:checked').length;
            const otherInput = document.getElementById('favoriteVegOther');
            const otherValue = otherInput ? otherInput.value.trim() : '';

            const veggieSelected = veggieCheckedCount > 0 || otherValue.length > 0;
            ...
            if (!veggieSelected) {
                showPopup('Please select at least one vegetable or enter your favourite in "Other".');
                return;
            }
    ...
    if (nextButton3) {
        nextButton3.addEventListener('click', () => {
            ...
            // Validate unstable vegetable selection
            if (typeof window.validateUnstableSelection === 'function') {
                if (!window.validateUnstableSelection()) {
                    return;
                }
            }
    ...
    if (nextButton4) {
        nextButton4.addEventListener('click', async () => {
            ...
            // Handle unstable veggies
            const unstableVeggies = [];
            document.querySelectorAll('input[name="unstableVeggie"]:checked').forEach(checkbox => {
                unstableVeggies.push(checkbox.value);
            });
            data.unstableVeggie = unstableVeggies;
    ...
    if (restartButton) {
        restartButton.addEventListener('click', () => {
            ...
            // Reset enhanced vegetable selection UI (favorite grid only)
            const veggieCards = document.querySelectorAll('#veggie-selection .veggie-card');
            veggieCards.forEach(card => {
                card.classList.remove('selected');
            });
